...
class ActorCriticController:
    ...
    def choose_action(self, state: np.ndarray) -> int:
        ...
        with self.tape:
            ...
            policy = self.actor_model(state, training=True)
    ...
    def learn(self, state: np.ndarray, reward: float, new_state: np.ndarray, terminal: bool) -> None:
        ...
        with self.tape:
            value = tf.squeeze(self.critic_model(state, training=True))
            next_value = tf.squeeze(self.critic_model(new_state, training=True))
        ...
        variables = self.actor_model.trainable_weights + self.critic_model.trainable_weights
        gradients = self.tape.gradient(total_loss, variables)
...
def init_training(learning_rate: float, epochs: int, model_name: str):
    try:
        environment = gym.make(model_name, render_mode="human")  # zamień na gym.make('LunarLander-v2', render_mode='human') by zająć się lądownikiem
        # zmień lub usuń render_mode, by przeprowadzić trening bez wizualizacji środowiska

        plots_path = f'plots/{model_name}_{learning_rate}_critic_author'
        models_path = f'models/{model_name}_{learning_rate}_critic_author'
        ...
        for i_episode in tqdm(range(epochs)):  # tu decydujemy o liczbie epizodów
            ...
            if i_episode % 25 == 0:  # tutaj o częstotliwości zrzucania wykresów
                ...
                plot_path = f'plots/{model_name}_{learning_rate}_critic_author/learning_{i_episode}.png'
                plt.savefig(plot_path)
                plt.clf()

                actor_model_path = f'models/{model_name}_{learning_rate}_critic_author/model_{i_episode}_actor.keras'
                critic_model_path = f'models/{model_name}_{learning_rate}_critic_author/model_{i_episode}_critic.keras'
                
                controller.actor_model.save(actor_model_path)
                controller.critic_model.save(critic_model_path)

        environment.close()
        
        actor_final_model_path = f"models/{model_name}_{learning_rate}_critic_author/final_actor.keras"
        critic_final_model_path = f"models/{model_name}_{learning_rate}_critic_author/final_critic.keras"
        
        controller.actor_model.save(actor_final_model_path)
        controller.critic_model.save(critic_final_model_path)
    except Exception as e:
        print(e)
        return None


init_training(0.0001, 2000, 'CartPole-v1')
init_training(0.00001, 2000, 'CartPole-v1')
