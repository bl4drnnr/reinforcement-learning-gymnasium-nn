...
def init_training(learning_rate: float, epochs: int, model_name: str):
    try:
        ...
        plots_path = f'plots/{model_name}_{learning_rate}_static'
        models_path = f'models/{model_name}_{learning_rate}_static'
        ...
        for i_episode in tqdm(range(epochs)):  # tu decydujemy o liczbie epizodów
            ...
            if i_episode % 25 == 0:  # tutaj o częstotliwości zrzucania wykresów
                ...
                plot_path = f'plots/{model_name}_{learning_rate}_static/learning_{i_episode}.png'
                plt.savefig(plot_path)
                plt.clf()

                model_path = f'models/{model_name}_{learning_rate}_static/model_{i_episode}.keras'
                controller.model.save(model_path)

        environment.close()

        final_model_path = f"models/{model_name}_{learning_rate}_static/final.keras"
