...
class ActorCriticController:
    ...
    def create_actor_critic_model() -> tf.keras.Model:
        ...
        input_layer = tf.keras.layers.Input(shape=(4,))  # 4 elementy stanu dla CartPole

        x = tf.keras.layers.Dense(128, activation='relu')(input_layer)
        x = tf.keras.layers.Dense(32, activation='relu')(x)

        actor_output = tf.keras.layers.Dense(2, activation='softmax')(x)  # 2 akcje
...
def init_training(learning_rate: float, epochs: int, model_name: str):
    try:
        ...
        plots_path = f'plots/{model_name}_{learning_rate}_static_small'
        models_path = f'models/{model_name}_{learning_rate}_static_small'
        ...
        for i_episode in tqdm(range(epochs)):  # tu decydujemy o liczbie epizodów
            ...
            if i_episode % 25 == 0:  # tutaj o częstotliwości zrzucania wykresów
                ...
                plot_path = f'plots/{model_name}_{learning_rate}_static_small/learning_{i_episode}.png'
                plt.savefig(plot_path)
                plt.clf()

                model_path = f'models/{model_name}_{learning_rate}_static_small/model_{i_episode}.keras'
                controller.model.save(model_path)

        environment.close()
        
        final_model_path = f"models/{model_name}_{learning_rate}_static_small/final.keras"
